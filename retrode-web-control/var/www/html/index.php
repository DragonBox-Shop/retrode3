...
if($slot=getvar('slot'))
	...
	if($result)
		...
		echo " ...done.</p>";
		flush();
		$path=str_replace("/usr/local/games/", "", $result);
		// vorläufig dirname damit die Datei gezeigt wird - oder gleich download=?
		text("Show File: ");
		weblink("$path", "files.php?file=".urlencode(dirname($path)));
		html("</p>");
		}
	else
		{
		echo " <font color=\"red\">...failed.</font></p>";
		// read /var/log/stdout.txt and stderr or give an option to show it
		}
	}
